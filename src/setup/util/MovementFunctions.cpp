...
void moveArc(double targetX, double targetY, double minError) {
  ...
  double fwd_kP = 4.6;   // 18
  double fwd_kD = 1.6; // 12.5
  double fwd_kI = 0; // 0.1

  double turn_kP = 4; // 4
  double turn_kI = 0;   // 0.06
  double turn_kD = 2; // 2

  double turnEffect = 3; //tune
  double fwdEffect = 3; //tune

  while (fwdError > minError) { // add counter back later
    // Calculating distances
    double targetAngle = getAngle(targetX, targetY);
    forwardDistance = getLength(targetX, targetY);
    ...
    turnError = (targetAngle - heading*180/M_PI);
    if (turnError < -180) {
    ...
    pros::lcd::print(3, "fwd error is %f", fwdError);
...
void forwardForDistance(double amount, double speed){
  ...
  while(DistanceTraveled < amount && counter < 350){
    ...
    DistanceTraveled = LeftRotation.get_position()*CONVERSION_FACTOR - current;
